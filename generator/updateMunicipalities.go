...
const (
  generatedFileName = "municipalities.go"
  baseURL           = "https://www.istat.it"

  municipalitiesURL      = baseURL + "/storage/codici-unita-amministrative/Elenco-comuni-italiani.csv"
  municipalityCodeColumn = 18
  municipalityNameColumn = 5

  countriesURL           = baseURL + "/it/files//2011/01/Elenco-codici-e-denominazioni-unita-territoriali-estere.zip"
  countriesFileExtension = ".csv"
  countryCodeColumn      = 9
  countryNameColumn      = 6
)
...
func main() {
  ...
  b, err := body(municipalitiesURL)
  if err != nil {
    log.Fatal(err)
  }
  err = populateMap(m, b, municipalityCodeColumn, municipalityNameColumn)
  ...
  for _, archiveFile := range archive.File {
    if path.Ext(archiveFile.Name) == countriesFileExtension {
  ...
  err = populateMap(m, b, countryCodeColumn, countryNameColumn)
  if err != nil {
    log.Fatal(err)
  }
